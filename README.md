...
| `link_color` | COLOR | `#2364f0` | 링크·활성 항목 색상 |
...
파랑, 메타 정보는 comment 회색을 씁니다. 원색 그대로는 아니고, 본문·코드 배경
어디에서도 대비 4.5:1(WCAG AA)을 넘도록 명도를 낮춰 잡았습니다. 편집기 테마는
흰 배경 대비를 기준으로 고른 색이 아니어서 그대로 쓰면 회색·노랑 계열이 걸립니다.

코드블록 안의 색은 `accent` / `link_color` 옵션을 따르지 않습니다. 사이트 색을
문법 팔레트에서 가져오는 방향이지 그 반대가 아니고, 옵션을 밝은 색으로 바꿨을 때
코드가 읽히지 않는 일을 막기 위해서입니다.
...
> 설치부터 옵션·커스터마이징·문제 해결까지 자세한 안내는
> **[설정 · 커스터마이징 안내](docs/SETUP.md)** 에 있습니다.
> 아래는 요약입니다.
...
`블로그 관리 > 꾸미기 > 스킨 편집 > 스킨 설정` 에서 위 표의 12개 옵션을 조정합니다.
...
전에도 자리를 차지합니다. 다만 광고가 **채워지지 않으면** 그 자리가 무너집니다
(아래 Known Limitations 참고).
...
| 폰트 | Pretendard + JetBrains Mono | 한국어 본문 최적화 + 코드·메타 정보용 고정폭. 가변 + 동적 서브셋, 렌더 비차단 |
...
- **광고 미충전 시 레이아웃 이동(CLS).** 광고 자리는 CSS 로 크기를 잡아 두지만,
  ...
  올라옵니다. 실측(2026-08-31)에서 이 한 번의 이동이 데스크톱 CLS 0.075,
  모바일 0.166 의 대부분이었습니다.
  바깥 래퍼에 `min-height` 를 걸어 자리를 붙잡는 방법을 시도했지만, 이동이
  줄기는커녕 늘었고(모바일 0.166 → 0.326) 광고가 없을 때 빈 칸이 남는 대가만
  확실해 채택하지 않았습니다. 본문 블록 자체가 늦게 나타나는 별도 원인이
  있는데 이건 티스토리가 `.post-content` 에 인라인 높이를 넣는 쪽이라
  스킨에서 손댈 수 없습니다.
